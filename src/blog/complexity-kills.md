...
1. **No slack.** There's no buffer between components. When one part fails, the next part in the chain feels it immediately.
2. **No substitution.** Components can't be swapped. If the only canal between the Mediterranean and the Red Sea is blocked, you can't use a different canal. There isn't one.
3. **Fixed sequences.** Operations must happen in a specific order. You can't skip steps or rearrange the process.
